...
export async function subirComprobante(id: number, archivo: File) {
  ...
  const response = await fetch(`https://yaolisbackend.vercel.app/comprobante/${id}`, {
    method: "POST",
    body: formData
  });
...
export async function actualizarEstado(id: number, nuevoEstado: string) {
    const response = await fetch(`https://yaolisbackend.vercel.app/admin/participantes/${id}/estado`, {
        method: "PUT",
        headers: {
